...
import { resolveSubmittedMentions } from "@/lib/comments/resolved-mentions";
...
/**
 * The `write-time mention resolution (data layer)` block that used to live here
 * has been DELETED, not moved.
 *
 * It drove mention recording through `recordMentions`, a hand-written mirror of
 * the notification block in `submitCommentAction`. A mirror is only as good as
 * its likeness, and this one drifted: it kept filtering self-mentions after the
 * action stopped doing so, which left the suite green while picking your own name
 * wrote no row. It also could not express the property that matters most —
 * whether the action AWAITS the row write before resolving — because it has no
 * notion of the action returning at all.
 *
 * `tests/server/comments.mention-rows.test.ts` drives the real action with only
 * its outside edges mocked, and gates the insert to pin that ordering.
 */
