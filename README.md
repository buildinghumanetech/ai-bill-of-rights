...
- **Comments do not migrate on their own.** `comments.base_version_id` points at the version a comment was written against, and the homepage only queries the current version — so without a migration step, publishing hides every existing thread. Ship a small SQL migration re-pointing them, as `drizzle/0010_repoint_comments_to_v0_1_0.sql` does for the 0.0.1 → 0.1.0 publish. **Run it after the deploy**, once `sync-versions` has created the new version row.

  **There are TWO different hazards here and they need different fixes.** Conflating them is how 0010 went wrong repeatedly:

  - **The anchor disappears** (a pill is renamed or removed). The comment survives in the database and renders nowhere. *Fixable by a migration* — remap the old anchor to the new one.
  - **The anchor survives but its TEXT changes.** Nothing to remap: the anchor is still there and still valid. The comment simply now sits under different words, so the author appears to have replied to something they never read. **A migration cannot fix this** — the only honest options are to accept it or to hide the comment, and either way it needs saying out loud.

  Both are silent. The stale anchor still resolves, the page still renders, nothing errors.

  **`pnpm test` catches both, via `tests/app/anchor-drift.test.ts`.** It compares `src/app/anchor-map.ts` — every anchor the homepage can emit, with the text it renders — against the committed snapshot of the published document in `tests/fixtures/published-anchor-map.json`. When it fails, that is the one moment anyone gets to decide what happens to the comments involved. Decide first, then `pnpm anchors:snapshot` to re-baseline. Running the snapshot first turns the tripwire into a rubber stamp.

  **There are FIVE anchor kinds, not two.** `HomepageArticles.tsx` emits `article-NN-title` (`:523`), `article-NN-s-M` over `splitSentences(article.body)` (`:544`), `article-NN-pullquote` (`:564`), `article-NN-connects-label` (`:583`) and `article-NN-connect-<slug>` (`:609`). 0010 reasoned about two of them, which is why Article 6's pull quote could change from `Every AI agent has a "license plate" identifier...` to "The loop stays open." — a completely different claim under a stable anchor — with nothing to catch it. No comment happened to be sitting there.

  **Sentence numbering comes from the PAGE, not the markdown.** `-s-M` indexes `splitSentences(article.body)`. The canonical markdown's `{#article-N-s-M}` ids count the pull quote as a body sentence and are unpadded, so the two disagree. 0010 was written against the markdown's numbering: it remapped `article-07-s-5` → `article-07-s-6` believing the COPPA line was inserted mid-article, when on the page COPPA was *appended* after s-4, the closing line is `article-07-pullquote`, and `s-6` does not exist at all. The remap matched zero rows. Read anchors off `anchor-map.ts`, never off the markdown.

  **"Connects to" pill slugs.** A comment can be anchored to a pill, as `article-NN-connect-<slug>`, so renaming a resource page orphans every comment on that pill. v0.1.0 renames nine (the HumaneBench pages take the benchmark's own principle names); 0010 remaps them. Derive the rename set from `git show <commit>:src/app/HomepageArticles.tsx` against HEAD, never from the working tree — otherwise you write mappings for pills that never shipped and miss ones that did.
  ...
  **What actually happened in the 0.0.1 → 0.1.0 publish**, recorded because the analysis above was wrong three times before it was right. Of the twelve anchors carrying comments in production, exactly one changed text: `article-04-s-1`, "persuasive dark patterns" → "deceptive patterns" — the same claim, sharper, so the two comments on it still read correctly. Nothing else moved. The pull-quote changes in Articles 1, 2 and 6 carried no comments, and the `article-07-s-5` remap matched nothing. The publish was safe, but that was luck, verified afterwards; `anchor-drift.test.ts` exists so the next one does not depend on someone thinking to check.
...
0011 backs the "propose a new right" flow at `/propose`. It adds `title`, `pull_quote`, `hidden_at` and `hidden_reason` to `proposed_edits` plus three indexes; no existing row is touched, and every statement is `IF NOT EXISTS`. **`/propose` reads and writes nothing until it has run** — `listProposedRights` selects `title`, so the page throws `column "title" does not exist` on an un-migrated database and falls into its empty-state catch. Safe to re-run.
